...
// The GrowthEventFunc type is an adapter to allow the use of ordinary
// function as GrowthEvent mutator.
type GrowthEventFunc func(context.Context, *ent.GrowthEventMutation) (ent.Value, error)

// Mutate calls f(ctx, m).
func (f GrowthEventFunc) Mutate(ctx context.Context, m ent.Mutation) (ent.Value, error) {
	if mv, ok := m.(*ent.GrowthEventMutation); ok {
		return f(ctx, mv)
	}
	return nil, fmt.Errorf("unexpected mutation type %T. expect *ent.GrowthEventMutation", m)
}
